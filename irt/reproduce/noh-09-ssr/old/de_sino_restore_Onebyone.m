...
sino_sel = 2; % 1 for decomp, 2 for PWLS, 3 for PL
if sino_sel == 1
    shat = de_ftab_s_iter(ftab.fit, fhat.raw, 'niter', 300);  % by LS
elseif sino_sel == 2
    regbeta.soft = 2^(-5);
    regbeta.bone = 0;
    [shat cost_eval] = de_ftab_s_pwls(ftab.fit, fhat.raw, ymi, 'niter', 300, 'beta', regbeta);  % by PWLS
    % [shat cost_eval] = de_ftab_s_pwls2(ftab.fit, fhat.raw, ymi,'niter', 300, 'beta', regbeta.soft);  %by PWLS
...
elseif sino_sel == 3
    regbeta.soft = 2^(-5); % reg for soft tissue
    regbeta.bone = 0; % reg for bone
    shat_pwls = de_ftab_s_pwls(ftab.fit, fhat.raw, ymi, 'niter', 40, 'beta', regbeta.soft);  % Init by PWLS
...
% Only low exposure level
% s_iter Nrms : 0.3162/0.8306

% s_pwls Nrms : beta = 2^(-5) 0.1336/0.3445
% s_pwls Nrms : beta = 2^(-3) 0.1141/0.3000
% s_pwls Nrms : beta = 2^(-4) 0.1202/0.3123
% s_pwls Nrms : beta = 2^(-6) 0.1547/0.3981

% s_pl Nrms : beta = 2^(-6) 0.1397/0.3592
% s_pl Nrms : beta = 2^(-5) 0.1234/0.3182
% s_pl Nrms : beta = 2^(-4) 0.1144/0.2970
% s_pl Nrms : beta = 2^(-3) 0.1119/0.2932
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Two Reg Parameters - beta.soft = 2^(-5)
% s_pl_Nrms : beta.bone = 2^(-15) 0.1280/0.3304
% s_pl_Nrms : beta.bone = 0  0.1280/0.3304
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Two Reg Parameters - beta.soft = 2^(-3)
% s_pl_Nrms : beta.bone = 0  0.0999/0.2585
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Two Reg Parameters - beta.soft = 2^(-2)
% s_pl_Nrms : beta.bone = 0 0.0968/0.2500
...
im(233, abs(xtrue(:, :, 1) - xfbp(:, :, 1)), '|Error|'); cbar([0 1]);
im(236, abs(xtrue(:, :, 2) - xfbp(:, :, 2))); cbar([0 1]);
...
% FBP_s_iter Nrms : 0.5408/0.6366

% FBP_s_pwls Nrms : beta = 2^(-5) 0.3265/0.4137
% FBP_s_pwls Nrms : beta = 2^(-3) 0.3315/0.4573
% FBP_s_pwls Nrms : beta = 2^(-4) 0.3141/0.4242
% FBP_s_pwls Nrms : beta = 2^(-6) 0.3917/0.4507
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Two Reg Parameters - beta.soft = 2^(-4)
% FBP_s_pwls_Nrms : beta.bone = 0 0.2729/0.3356

% FBP_s_pl Nrms : beta = 2^(-3) 0.3448/0.4705
% FBP_s_pl Nrms : beta = 2^(-4) 0.3203/0.4346
% FBP_s_pl Nrms : beta = 2^(-5) 0.3131/0.4106
% FBP_s_pl Nrms : beta = 2^(-6) 0.3473/0.4205
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Two Reg Parameters - beta.soft = 2^(-5)
% FBP_s_pl_Nrms : beta.bone = 2^(-8) 0.2942/0.3585
% FBP_s_pl_Nrms : beta.bone = 2^(-9) 0.2943/0.3554
% FBP_s_pl_Nrms : beta.bone = 2^(-10) 0.2944/0.3539
% FBP_s_pl_Nrms : beta.bone = 2^(-12) 0.2946/0.3529
% FBP_s_pl_Nrms : beta.bone = 2^(-15) 0.2947/0.3526
% FBP_s_pl_Nrms : beta.bone = 0 0.2947/0.3525
% For pl, as beta.bone decreases, Nrms erros decreases.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Two Reg Parameters - beta.soft = 2^(-3)
% FBP_s_pl_Nrms : beta.bone = 0 0.2539/0.3325
% Two Reg Parameters - beta.soft = 2^(-4)
% FBP_s_pl_Nrms : beta.bone = 0 0.2566/0.3289
