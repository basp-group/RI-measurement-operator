...
ig.mask = logical(ig.circ(max(x_mask, y_mask))); %% by dh
if x_mask ~= y_mask
    % warn 'need to put in ellipse using ellipse_im'
    ig.mask = logical(ellipse_im(ig)).'; %% by dh
end
...
% w_0(Phi)
% [wknot, tw] = adw_fan(G.arg, wi, mask, Phi); %% by dh
[wknot, tw] = adw_fan_new(sg, ig, wi); %% by dh
...
% Jj = G.arg.dis_src_iso/G.arg.dis_src_det; %% by dh
Jj = sg.dso / sg.dsd; %% by dh
...
%   std_fft = fld_read([f.data_dir 'std_vh,dbsized_mask,iter71,ntime250,l2b11,256,nblock41,kappa.fld']);
    ...
    sgcal = sino_geom('fan', ...
        ...
        'orbit_start', f.orbit_start, ...
        'offset_s', 0);
    igcal = image_geom('nx', nx, 'ny', ny, 'dx', f.pixel_size);
    igcal.mask = maskcal;
    if x_mask ~= y_mask
        % warn 'need to put in ellipse using ellipse_im'
    end
    Gcal = Gtomo2_wtmex(sgcal, igcal);
    %% by dh

if 0 %% this is an old way by dh
    ...
    % [wknotcal, tw] = adw_fan(G.arg, ones(size(wi)), mask, Phi); %% by dh
    [wknotcal, tw] = adw_fan_new(sgcal, igcal, ones(size(wi))); %% by dh
    CC = (dx .* dy) ./ (ds);
    KK = CC / (rhomax)^3;
    for indx = 1:length(c.beta)
...
figure; hold on;
plot(std_pre(:, 128)); plot(std_pre1(:, 128), ':');
xlabel 'pixels'; ylabel 'std. dev.'; title 'horizontal profiles for case2';
legend('DI prediction', 'SI prediction'); axis([0 256 0 4 * 10^-5]);

figure; hold on;
plot(std_pre(128, :)); plot(std_pre1(128, :), ':');
xlabel 'pixels'; ylabel 'std.dev.'; title 'vertical profiles for case2';
legend('DI prediction', 'SI prediction'); axis([0 256 0 4 * 10^-5]);
