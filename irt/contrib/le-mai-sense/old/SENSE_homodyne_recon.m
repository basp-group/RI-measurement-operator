function [demod_recon_im, recon_im, lp_im] = SENSE_homodyne_recon(reduced_ffts, smap, overlap, ...
    homodyne_direction, SENSE_direction, regularizer, nc, np, red_dims, figs)
...
recon_im = SENSE_recon(smap, aliased_recon_ffts, SENSE_direction, np, regularizer, figs);
lp_im = SENSE_recon(smap, aliased_lp_ffts, SENSE_direction, np, regularizer, figs);

% phase demodulation
demod_recon_im = real(recon_im .* exp(-i * angle(lp_im)));
