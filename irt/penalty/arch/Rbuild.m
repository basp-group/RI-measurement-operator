...
%   potential       e.g. 'huber', see penalty_funcs()
%   delta           potential parameter, see penalty_funcs()
...
warning('Rbuild still works but has been superceded by Robject');
...
%
% inline functions associated with desired potential function
% trick: empty 'beta' means weighted version!
%
[R.pot R.wpot R.dpot] = penalty_funcs(potential, [], delta);
...
if R.isquad
    ...
    R.penal = inline('sum(R.pot(R.wt, R.C * x))', 'R', 'x');
    R.cgrad = inline('R.C'' * R.dpot(R.wt, R.C * x)', 'R', 'x');
    if R.has_sps
        R.denom = inline('R.E * R.wpot(R.wt, R.C * x)', 'R', 'x');
    end
    R.diag = inline('transpose(abs(R.C.^2))*R.wpot(R.wt, 0)', 'R');
end
