...
for id=1:dd
	...
	elseif streq(ktype, 'function_handle')
		...
		tmp = nufft_scale(Nd(id), Kd(id), st.alpha{id}, st.beta{id}); % [PA] active line by default
	end
	st.sn = st.sn(:) * tmp';
end
if length(Nd) > 1
...
for id=1:dd
	...
	koff = nufft_offset(om(:,id), J, K); % [M 1] to leftmost near nbr  % [PA]: computes the matrix indices of the leftmost (first) element in the neighbourhood centered in the given frequency locations ()
    kd{id} = mod(outer_sum([1:J]', koff'), K) + 1; % [J? M] {1,...,K?} % [PA]: computes the indices of the elements in the region centered around the koff frequencies: just add 1:J, frequencies in {1,...,K} (formula (9)))
	if id > 1 % trick: pre-convert these indices into offsets!
		kd{id} = (kd{id}-1) * prod(Kd(1:(id-1))); % [PA] change reference (indices in the final large matrix G)
	end
...
uu = conj(uu) .* phase(ones(1,prod(Jd)),:); % [*Jd M] % [PA] use bsxfun instead of duplicating entries... 
...
% st.p = Gsparse(st.p, 'odim', [M 1], 'idim', [prod(Kd) 1]); % [PA] commented, as requested by Alex -> Gsparse structure has indexing issues
